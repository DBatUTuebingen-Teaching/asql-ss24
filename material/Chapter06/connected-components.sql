...
CREATE SEQUENCE node_id;

CREATE TABLE nodes (
  node  int DEFAULT nextval('node_id') PRIMARY KEY,
  label text);
...
walks(iter, node, front) AS (
  ...
  FROM   walks AS w, graph AS g
  WHERE  w.front = g."from"
  AND    w.iter < 10 -- allow for plenty of steps
)
SELECT w.node, w.front, MIN(w.iter) AS "encountered at step"
FROM   walks AS w
GROUP BY w.node, w.front
ORDER BY "encountered at step", w.node, w.front;
